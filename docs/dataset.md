...
`backend/app/synthetic.py` generates deterministic, reproducible fraud-ring
scenarios with ground-truth labels for evaluation. It is the primary dataset
for AriadneThread.

`backend/app/data_gen.py` is preserved for the streaming demo and
...
Ground-truth labels (`is_fraud_ring_member`, `ring_id`, `typology`, `role`)
are separated from transaction data before writing `transactions.csv`. This
prevents models from leaking on these columns. Ground truth is stored in a
separate `ground_truth.json` for evaluation only.

## External Data (Optional)

An optional IBM AMLworld dataset can be downloaded for additional realism:
...
3. **Constructive counterfactuals** — adversarial testing
4. **Temporal split** — train on T-30d, test on T-0d

Evaluation metrics: Precision ≥ 0.80, Recall ≥ 0.60, F1 ≥ 0.65, Explanation
faithfulness ≥ 0.90.
